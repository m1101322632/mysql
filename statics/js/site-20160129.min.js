    ...
    $.doc = function(opt) {
        ...
        this.init = function() {
            ...
            $('#expand-footer').click(function() {
                ...
                if ($('footer').hasClass('collapsed')) {
                    ...
                    $('#footer-links').show();
                    span.removeClass('icon-chevron-up').addClass('icon-chevron-down');
                    ...
                    span.removeClass('icon-chevron-down').addClass('icon-chevron-up');
            ...
            if (!this.options.mobile) {
                $('pre.programlisting').hover(function() {
                    var pre = this;
                    if ($(pre).find('.docs-select-all').length == 0) {
                        var id = 'sa' + Math.floor((Math.random() * 100000000) + 1);
                        var cmd = (navigator.appVersion.indexOf("Mac") != -1) ? '&#8984;+C' : 'CTRL+C';
                        var div = '<div class="docs-select-all"><div class="copy-help left">Press ' + cmd + ' to copy</div> <div class="right"><a href="" title="Select All" id="' + id + '"><span class="icon-clipboard"></span></a></div></div>'
                        $(pre).prepend($(div));
                        $('#' + id).click(function() {
                            var saDiv = $(pre).find('.docs-select-all')[0];
                            var chDiv = $(pre).find('.copy-help')[0];
                            if (window.getSelection) {
                                var selection = window.getSelection();
                                var range = document.createRange();
                                range.selectNodeContents(pre);
                                range.setStartAfter(saDiv);
                                selection.removeAllRanges();
                                selection.addRange(range)
                            } else if (document.body.createTextRange) {
                                var range = document.body.createTextRange();
                                range.moveToElementText(pre);
                                range.moveStart('character', $(saDiv).text().length);
                                range.select()
                            }
                    ...
                    $(pre).find('div:first').show();
                    self.adjustHeight()
                }, function() {
                    window.clearTimeout(self.copyTimer);
                    self.copyTimer = null ;
                    $(this).find('.copy-help').hide();
        ...
        this.scrollThead = function() {
            ...
            $(this.tables).each(function() {
                var w, s = 'thead tr th, thead tr td', t = $('table', this.parent().prev()).get(0), c = $('caption', t), collapse = $(t).css('border-collapse') == 'collapse', ths = $(s, t), offset = self.document.scrollTop() - $(t).offset().top + self.theadMagicNumber;
                if (c.length) {
        ...
        this.initComments = function(prefix) {
            ...
            $(prefix + ' .docs-comment-cancel-btn').click(function(e) {
                var id = $(this).closest('form').find('.id-input').val();
                $('#c' + id + ' .docs-comment-item-edit').html();
                $('#c' + id + ' .docs-comment-item-edit').hide();
                $('#c' + id + ' .docs-comment-item-text').show();
                self.adjustHeight();
                return !1
            });
            ...
            $(prefix + ' .docs-comment-edit-link').click(function(e) {
                var id = $(this).attr('id').substring(1);
                self.showCommentLinkSpinner(id);
                $.ajax({
                    type: 'GET',
                    ...
                    success: function(data, msg) {
                        if (!data.success) {
                            ...
                            $('#c' + id + ' .docs-comment-item-edit').html(data.results).show();
                            var path = $('#add-comment .path-input').val();
                            $('#c' + id + ' .path-input').val(path);
                            self.initComments('#c' + id + ' .docs-comment-item-edit');
                            $('#c' + id + ' .docs-comment-item-text').hide();
                            self.adjustHeight()
                        }
                        self.hideCommentLinkSpinner(id)
                    },
                    ...
                    success: function(data, msg) {
                        if (!data.success) {
                            ...
                            if (id == '') {
                                ...
                                $('#c' + id).replaceWith(data.results);
                                self.initComments('#c' + id)
                            }
                            self.adjustHeight()
                        }
...
(function($) {
    $.tco = function(opt) {
        ...
        this.refresh = function(top) {
            var self = this;
            var msg = this.options.lead_captured ? 'Calculating' : 'Validating Selections';
            if (top) {
    ...
    if (!$.browser.mobile) {
        ...
        var stickyNav = function() {
            if (docTocObj.length) {} else {
                if ($(window).scrollTop() > headerObj.height()) {
                    var width = $(window).width();
                    if ((!navPosition || navPosition == 'static') && width > navStickyMinWidth) {
                        navObj.addClass('sticky');
                        bodyObj.addClass('has-sticky-header').css('padding-top', navObj.height() + 'px');
                        navPosition = 'fixed'
